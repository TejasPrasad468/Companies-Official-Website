<?php
/**
 * The Core Functions for SKT Ele Product Launch
 *
...
// Shortcode Promobox1
/* [promobox1 bgcolor="#f7f7f7" topbordercolor="#2f5bea" otherbordercolor="#e8e6e6"]Description[/promobox1] */
...
// Shortcode Promobox2
/* [promobox2 bgcolor="#f7f7f7" leftbordercolor="#2f5bea" button="Hello Text" url="#"]Description[/promobox2] */
...
// Shortcode Promobox3
/* [promobox3 bgcolor="#f7f7f7" bottombordercolor="#2f5bea" button="Hello Text" url="#"]Description[/promobox3] */
...
// Shortcode Promobox4
/* [promobox4 bgcolor="#f7f7f7" bordercolor="#2f5bea"]Description[/promobox4] */
  ...
  <div class="skt-home-posts-row">
	...
	<div class="skt-home-posts-column">
    	<div class="skt-home-posts-inner">
            ...
            <a class="skt-home-posts-overlay" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"></a>
            <div class="skt-home-posts-content">
            	<h3 class="skt-home-posts-title"><?php the_title(); ?></h3>
                <span class="skt-home-posts-date"><?php the_time('d F, Y'); ?> - <?php the_category(', '); ?></span>
            </div>
        </div>
     </div>
...
function my_custom_post_project() {
  $labels = array(
    'name'               => __( 'Our Project','complete'),
    'singular_name'      => __( 'Our Project','complete'),
    'add_new'            => __( 'Add Our Project','complete'),
    'add_new_item'       => __( 'Add New Our Project','complete'),
    'edit_item'          => __( 'Edit Our Project','complete'),
    'new_item'           => __( 'New Our Project','complete'),
    'all_items'          => __( 'All Our Project','complete'),
    'view_item'          => __( 'View Our Project','complete'),
    'search_items'       => __( 'Search Our Project','complete'),
    'not_found'          => __( 'No Our Project found','complete'),
    'not_found_in_trash' => __( 'No Our Project found in the Trash','complete'), 
    'parent_item_colon'  => '',
    'menu_name'          => 'Our Project'
  );
  $args = array(
    'labels'        => $labels,
    'description'   => 'Manage Our Project',
    'public'        => true,
  ...
  register_post_type( 'ourproject', $args );  
}
add_action( 'init', 'my_custom_post_project' );


// add meta box to ourtour
add_action( 'admin_init', 'my_ourproject_admin_function' );
function my_ourproject_admin_function() {
    add_meta_box( 'ourproject_meta_box',
        'Our Project Info',
        'display_ourproject_meta_box',
        'ourproject', 'normal', 'high'
    );
}
// add meta box form to doctor
function display_ourproject_meta_box( $ourproject ) {
    // Retrieve current name of the Director and Movie Rating based on review ID
    $companyproject = esc_html( get_post_meta( $ourproject->ID, 'companyproject', true ) );  
  
    ?>
    <table width="100%">
        <tr>
            <td width="20%">Company </td>
            <td width="80%"><input size="80" type="text" name="companyproject" value="<?php echo $companyproject; ?>" /></td>
        </tr> 
    </table>
    <?php    
}
// save ourtour meta box form data
add_action( 'save_post', 'add_ourproject_fields_function', 10, 2 );
function add_ourproject_fields_function( $ourproject_id, $ourproject ) {
    // Check post type for ourtour
    if ( $ourproject->post_type == 'ourproject' ) {
        // Store data in post meta table if present in post data
    if ( isset($_POST['companyproject']) ) {
            update_post_meta( $ourproject_id, 'companyproject', $_POST['companyproject'] );
        }       
    }
}

//[ourproject-rotator show="4"]
//Our Tour function
function ourproject_rotator_output_func( $atts ){
   extract( shortcode_atts( array(
    'show' => '4',
  ), $atts ) );
    extract( shortcode_atts( array( 'show' => $show,), $atts ) ); 
  
  $ourprojectoutput = '<div class="bxmain rota ourtour">
  <ul class="bxslider" style="display: none;">';
  wp_reset_query();
   $args = array( 'post_type' => 'ourproject', 'posts_per_page' => $show, 'orderby' => 'date', 'order' => 'desc' );
  query_posts( $args );
  if ( have_posts() ) :
    while ( have_posts() ) : the_post();
    $companyname = esc_html( get_post_meta( get_the_ID(), 'companyproject', true ) );
      $ourprojectoutput .= '
       <li>	    
        <div class="ourtour-image"><a href="'.get_permalink().'">'.get_the_post_thumbnail( get_the_ID(), array() ).'</a></div>
      ...
      ';
    endwhile;
     $ourprojectoutput .= '</ul></div>';
  else:
    $ourprojectoutput = ' 
     ';     
    endif;  
  wp_reset_query();
  return $ourprojectoutput;
}
add_shortcode( 'ourproject-rotator', 'ourproject_rotator_output_func' );
